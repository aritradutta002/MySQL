...
select employee_id , first_name , last_name , salary, phone_number, email from employee_details where 
job_id like '%AC_ACCOUNTANT%' and salary > all (select salary from employee_details where job_id like '%AD_VP%');
...
where 
datediff((select max(hire_date) from employee_details),hire_date)  >=  (select datediff(max(hire_date),min(hire_date))/2 from employee_details);
...
select employee_id , first_name , last_name, salary, job_id from employee_details e 
where hire_date in (select min(hire_date) from employee_details e1 where e.department_id = e1.department_id);
...
select employee_id , first_name , last_name, salary, job_id from employee_details e
where salary in (select max(salary) from employee_details e1 where e.job_id=e1.job_id);